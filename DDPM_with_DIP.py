...
def generate_samples_and_evaluate_fid(model, dataloader, device, num_samples=50):
...
def train_DDPM_with_DIP():
    ...
    dip_model.load_state_dict(torch.load('./result/dip_model.pth'))  # 加載已訓練的 DIP 模型
    dip_model.eval()
            ...
            for data in dataloader:
                ...
                if step % 3000 == 0:
                    ...
                    if avg_loss < best_loss:
                        best_loss = avg_loss
                        torch.save(ddpm_model.state_dict(), 'result/ddpm_dip_model.pth')
    ...
    plt.title('DDPM with DIP Training Loss Over Steps')
    plt.xlabel('Step (3000)')
    plt.ylabel('Loss')
    plt.grid(True)
    plt.savefig('./figure/ddpm_dip_training_loss.png')
    plt.show()

    plt.figure()
    plt.plot(range(1, len(fid_scores) + 1), fid_scores, marker='o')
    plt.title('DDPM with DIP FID Scores Over Steps')
    plt.xlabel('Step (3000)')
    plt.ylabel('FID Score')
    plt.grid(True)
    plt.savefig('./figure/ddpm_dip_fid_scores.png')
    plt.show()
